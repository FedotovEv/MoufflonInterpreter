...
namespace runtime
{
    ...
    // Точка останова может быть позиционной (установленной на конкретную позицию исходного текста программы) или непозиционной (делокализованной).
    // Бряк второго типа срабатывает при возникновении определённых условий в процессе выполнения программы, которые могут сложиться в любом её месте,
    // а не только в заранее выбранной позиции.
    // В настоящее время поддерживаются следующие такие элементарные (простейшие) события, которые могут вызвать срабатывание точек останова второго типа:
    // 1. Инстанцирование объекта класса.
    // 2. Разрушение объекта класса.
    // 3. Доступ на чтение.
    // 4. Доступ на запись к существующей переменной (перезапись существующей переменной).
    // 5. Создание новой переменной (доступ на запись к переменной, которой до того не существовало).
    // 6. Удаление переменной (исполнение инструкции del).
    // Также возможно произвольное комбинирование этих элементарных событий, образуя этим способом иные, более сложные, составные события и их сочетания.
    // Наиболее распространённым таким сочетанным событием будет, вероятно, произвольный доступ к переменным и полям, представляющим собой комбинацию
    // всех возможных операций доступа к переменной или полю - чтения, записи (как новой, так и прежде существующей), а также её удаления.
    // ---------
    // Кроме типа, выражающего собой общую характеристику сущности произошедшего события, события также подразделяются на три категории, указывающих
    // совокупность объектов, которых такое событие затрагивает. Определены следующие категории событий:
    // 1. Объектное событие.
    // 2. Событие для переменной (поля).
    // 3. Классовое событие. 
    //   События первой категории ассоциированы с конкретным существующим в памяти объектом, имеющим там свой определённый адрес. Внутренне ("с точки
    // зрения" исполняющей среды) такое событие привязывается к некоторому отдельному контейнеру класса ObjectHolder, существующему в динамической
    // памяти (куче). Данные события распространяется на любые обращения к такому объекту, вне зависимости от того, посредством какой именно
    // переменной или ссылки такое обращение выполняется.
    //   События второй категории привязаны к определённой переменной и считаются состоявшимися, только если имеется прямое или косвенное использование
    // именно той переменной, имя которой соответствует шаблону, указаному в его описании. Также эти события активируются в случае косвенного
    // использования такой переменной, то есть её использования посредством ссылки на неё (с помощью объекта класса runtime::PointerObject).
    //   Наконец, события третьей категории считаются состояшимися при обращении к любому объекту некоторого класса, имя которого равно содержимому
    // именного шаблона из описания события.
    //   Каждое событие, таким образом, характеризуется своим типом, а также набором объектов, с которым оно ассоциировано, а этот набор, в свою
    // очередь, определяется категорией и именным шаблоном, который может быть либо обобщённым (содержать один или несколько джокеров и соответствовать
    // сразу нескольким объектам), либо конкретным (не содержать символов обобщения и указывать только на один определённый объект). И каждому
    // распределённому (нелокализованному) бряку при его создании назначается одно такое событие, при выполнении условий которого происходит его
    // срабатывание.

    enum BreakpointEventType
    {
        BREAK_EVENT_TYPE_UNKNOWN = 0,       // Зарезервированное значение-заполнитель для неактивных или невалидных описаний точек останова.
        BREAK_EVENT_TYPE_INSTANTIATE = 1,   // Создание (инстанцирование) объекта класса.
        BREAK_EVENT_TYPE_DESTROY = 2,       // Разрушение объекта класса.
        BREAK_EVENT_TYPE_READ = 4,          // Чтение переменной либо поля объекта или класса.
        BREAK_EVENT_TYPE_WRITE = 8,         // Перезапись существующей переменной либо поля объекта или класса.
        BREAK_EVENT_TYPE_CREATE = 16,       // Создание новой переменной или поля объекта (или всех объектов некоторого класса).
        BREAK_EVENT_TYPE_DELETE = 32        // Удаление переменной или поля объекта (или, возможно, класса).
    };

    enum class BreakpointEventCategory
    {
        BREAK_EVENT_CAT_UNKNOWN = 0,        // Зарезервированный заполнитель.
        BREAK_EVENT_CAT_OBJECT,             // Событие объектной категории.
        BREAK_EVENT_CAT_VAR,                // Событие категории переменной.
        BREAK_EVENT_CAT_CLASS               // Событие классовой категории.
    };

    struct LocalBreakpointDesc
    {
        ProgramCommandDescriptor position{-1, -1};  // Положение точки останова в исходном коде МУФЛОН-программы.
    };

    struct UnlocalBreakpointDesc
    {
        BreakpointEventType event_type = BreakpointEventType::BREAK_EVENT_TYPE_UNKNOWN;
        BreakpointEventCategory event_category = BreakpointEventCategory::BREAK_EVENT_CAT_UNKNOWN;
        std::string mask;   // Именной шаблон события точки останова.
    };

    using BreakpointVarDesc = std::variant<std::monostate, LocalBreakpointDesc, UnlocalBreakpointDesc>;

    struct BreakpointDesc
    {
        // Часть описания бряка, имеющая специфическую структуру для каждой разновидности точек останова.
        BreakpointVarDesc var_desc;
        // Универсальные поля описания бряка, задействованные для точек останова любого типа.
        ...
        bool IsValid() const
        {
            return !std::holds_alternative<std::monostate>(var_desc);
        }

        bool IsLocal() const
        {
            return std::holds_alternative<LocalBreakpointDesc>(var_desc);
        }

        bool IsUnlocal() const
        {
            return std::holds_alternative<UnlocalBreakpointDesc>(var_desc);
        }
    };
    ...
    public:
        ...
        // -----------
        // Методы создания различных локальных (позиционных) точек останова.
        // Создание типового позиционного бряка на некоторое положение в коде. Все его параметры, кроме положения, принимаются по умолчанию.
        size_t AddPositionBreak(const ProgramCommandDescriptor& new_break_position);
        ...
        // -----------
        // Методы создания различных нелокальных точек останова.
        // Установка бряка на создание и/или уничтожение объектов класса class_name.
        size_t AddInstantiateBreak(const std::string& class_name, bool is_instantiate = true, bool is_destroy = true);
        ...
        size_t AddBreakVarReadWrite(const std::string& var_name, bool is_var_read = true, bool is_var_write = true);
        // Создание бряка на создание/удаление конкретной переменной. Правила задания имени var_name аналогичны вышеобъявленной функции.
        size_t AddBreakVarCreateDestroy(const std::string& var_name, bool is_var_create = true, bool is_var_delete = true);
        // -----------
    ...
    private:
        static constexpr BreakpointDesc DUMB_BREAKPOINT{.var_desc = {}, .break_count = 0, .is_conditional = false, .is_enabled = false/*, .is_passed = true*/};
        ...
        // Формирование списка локальных (привязанных к определённому месту исходника) точек останова для исходной строки test_break_position,
        // которые должны сработать в данный момент.
        size_t FindLocalBreakpoints(const ProgramCommandDescriptor& test_break_position, Executable* exec_statement, Closure& closure);
