...
------------------------
  ...
  Простейший  пример  использования  интерпретатора  можно  обнаружить  в  файле
main.cpp.   Там   же   находятся   модульные   тесты,  проверяющие  корректность
исполнения   некоторых   основных  конструкций  языка  Муфлон.  После  успешного
завершения   всех   тестов   можно  вводить  программный  код  на  языке  Муфлон
непосредственно в консоль, получая туда же результаты его исполнения.

  Примеры  применения  интерпретатора   в   качестве   встроенного   скриптового
языка   находятся   в   подкаталогах  ExternalObjectDemo  и  NonStdPrint.  Более
подробно  как  сам  язык,  так  и  возможные способы его использования описаны в
этом документе далее.
